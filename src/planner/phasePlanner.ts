...
- [ ] _(tasks will be generated by \`onyx plan "<projectId>" <n>\`)_
...
// Step 1: call LLM with Overview + repo scan → create P1..Pn phase notes in ONYX format.
...
Use the phase note format from your system prompt. Write files directly.`;
    ...
    if (result.success && fs.existsSync(phasesDir)) {
      ...
      console.warn('[onyx] Agent plan returned success but no phase files found — falling back to JSON path');
    }
...
<!-- AGENT_WRITABLE_START:phase-plan -->
- [ ] _(tasks will be generated by \`onyx plan "${bundle.projectId}" ${n}\`)_
<!-- AGENT_WRITABLE_END:phase-plan -->
...
- ${today} **phase_created** — Phase created by \`onyx plan\`
`);
...
//
// Writes P(N+1)..P(N+k) as backlog phase stubs ready for `onyx plan <project> <n>`.
...
Use the phase note format from your system prompt. Write files directly.`;
    ...
    if (result.success && fs.existsSync(phasesDir)) {
      ...
      console.warn('[onyx] Agent extend returned success but no new phase files found — falling back to JSON path');
    }
    // Fall through to JSON path
  }
...
- ${today} phase_created — extended by \`onyx plan --extend\`
`;
...
- ${today} **phase_created** — Phase extended by \`onyx plan --extend\`
`);
